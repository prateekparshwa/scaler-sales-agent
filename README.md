...
A Next.js app that gives a Scaler BDA two superpowers in one cockpit. The BDA pastes a lead profile (or loads one of the three standard personas) plus either a typed transcript or an uploaded call recording, and gets back two artefacts: (a) a short, persona-tuned **pre-call WhatsApp nudge** that lands on their own WhatsApp ahead of the dial — telling them who the lead is, two-three angles to lead with, two-three objections to expect, and a suggested opening hook; and (b) a **2–3 page personalised PDF** answering each open question the lead actually raised on the call, with claims grounded in a pre-embedded RAG corpus over scaler.com and explicit refusal to fabricate when the corpus is silent. The PDF goes through an **Approve / Edit cover / Skip** gate before any lead-facing send — only the BDA-only nudge auto-fires. Approve → the PDF + a personalised cover note land on the lead's WhatsApp via Twilio. Both text and audio input modes work end-to-end on the deployed link.
